...
test('Buffering Logical Display Descriptor (LDD)', async (t) => {
    ...
    await t.test("Testing LDD sizes", () => {
        ...
        assert.strictEqual(width.equals(expectWidth), true, "After buffering LDD width is not equals expected values");
        assert.strictEqual(height.equals(expectHeight), true, "After buffering LDD width is not equals expected values");
    })
    ...
    await t.test('Check packed fields', () => {
        ...
        assert.strictEqual(expectedValue.equals(value), true, 'Packed fields are not equal expected value')
    })

    await t.test('Background color index', () => {
        const expectedValue = Buffer.from(new Uint8Array([0]).buffer);
        assert.strictEqual(buffer.subarray(0x5, 0x6).equals(expectedValue), true, "Background color index is not equals to expected values");
    })
    ...
    await t.test('Pixel Aspect Ratio', () => {
        ...
        assert.strictEqual(expectedValue.equals(value), true, "Pixel Aspect Ration not equals expected value");
    })
