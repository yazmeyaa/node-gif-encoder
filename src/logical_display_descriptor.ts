...
type DefaultLDDValues = Required<
    Pick<
        LogicalDisplayDescriptorConstructor,
...
export class LogicalDisplayDescriptor implements BytesEncoder {
    ...
    constructor(data: LogicalDisplayDescriptorConstructor) {
        ...
        if (data.width > maxSizeValue || data.height > maxSizeValue)
            throw new Error(
                `Unsupported virtual display resolition. Expected values in range (0..${maxSizeValue})`,
            );
        this.width = data.width;
        this.height = data.height;
        this.globalColorTableFlag =
            data.globalColorTableFlag ??
            DEFAULT_LOGICAL_DESCRIPTOR_VALUES.globalColorTableFlag;
        this.backgroundColorIndex =
            data.backgroundColorIndex ??
            DEFAULT_LOGICAL_DESCRIPTOR_VALUES.backgroundColorIndex;
        this.pixelAspectRatio =
            data.pixelAspectRatio ??
            DEFAULT_LOGICAL_DESCRIPTOR_VALUES.pixelAspectRatio;
        this.sortFlag =
            data.sortFlag ?? DEFAULT_LOGICAL_DESCRIPTOR_VALUES.sortFlag;
        this.colorResolution =
            data.colorResolution ??
            DEFAULT_LOGICAL_DESCRIPTOR_VALUES.colorResolution;
        this.sizeOfGlobalColorTable =
            data.sizeOfGlobalColorTable ??
            DEFAULT_LOGICAL_DESCRIPTOR_VALUES.sizeOfGlobalColorTable;
    }
